...
#
#   $foo_monitor_port    (true|false) : Set if you want to monitor foo's port(s). If any.
#                                       Default: As defined in $monitor_port
#   $foo_monitor_url     (true|false) : Set if you want to monitor foo's url(s).  If any.
#                                       Default: As defined in $monitor_url
#   $foo_monitor_process (true|false) : Set if you want to monitor foo's process. If any.
#                                       Default: As defined in $monitor_process
#   $foo_monitor_plugin  (true|false) : Set if you want to monitor foo using specific monitor tool's plugin. If any.
#                                       Default: As defined in $monitor_plugin
#   $foo_monitor_target               : Define how to reach (Ip, fqdn...) the host to monitor foo from an external server.
#                                       Default: As defined in $monitor_target
#   $foo_monitor_url                  : Define the baseurl (http://$fqdn/...) to use for eventual foo URL checks.
#                                       Default: As defined in $monitor_url
#
...
class foo::monitor {
    ...
    monitor::process { "foo_process":
        process  => "${foo::params::process_name}",
        service  => "${foo::params::service_name}",
