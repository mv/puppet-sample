...
#
#   $ntp_monitor_port    (true|false) : Set if you want to monitor ntp's port(s). If any.
#                                       Default: As defined in $monitor_port
#   $ntp_monitor_url     (true|false) : Set if you want to monitor ntp's url(s).  If any.
#                                       Default: As defined in $monitor_url
#   $ntp_monitor_process (true|false) : Set if you want to monitor ntp's process. If any.
#                                       Default: As defined in $monitor_process
#   $ntp_monitor_plugin  (true|false) : Set if you want to monitor ntp using specific monitor tool's plugin. If any.
#                                       Default: As defined in $monitor_plugin
#   $ntp_monitor_target               : Define how to reach (Ip, fqdn...) the host to monitor ntp from an external server.
#                                       Default: As defined in $monitor_target
#   $ntp_monitor_url                  : Define the baseurl (http://$fqdn/...) to use for eventual ntp URL checks.
#                                       Default: As defined in $monitor_url
#
