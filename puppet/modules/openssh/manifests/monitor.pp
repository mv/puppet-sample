...
#
#   $openssh_monitor_port    (true|false) : Set if you want to monitor openssh's port(s). If any.
#                                       Default: As defined in $monitor_port
#   $openssh_monitor_url     (true|false) : Set if you want to monitor openssh's url(s).  If any.
#                                       Default: As defined in $monitor_url
#   $openssh_monitor_process (true|false) : Set if you want to monitor openssh's process. If any.
#                                       Default: As defined in $monitor_process
#   $openssh_monitor_plugin  (true|false) : Set if you want to monitor openssh using specific monitor tool's plugin. If any.
#                                       Default: As defined in $monitor_plugin
#   $openssh_monitor_target               : Define how to reach (Ip, fqdn...) the host to monitor openssh from an external server.
#                                       Default: As defined in $monitor_target
#   $openssh_monitor_url                  : Define the baseurl (http://$fqdn/...) to use for eventual openssh URL checks.
#                                       Default: As defined in $monitor_url
#
