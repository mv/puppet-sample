# Class snmp::monitor
#
# Monitors snmp process/ports/service using Example42 monitor meta module (to be adapted to custom monitor solutions)
#
...
#
#   $snmp_monitor_port    (true|false) : Set if you want to monitor snmp's port(s). If any.
#                                       Default: As defined in $monitor_port
#   $snmp_monitor_url     (true|false) : Set if you want to monitor snmp's url(s).  If any.
#                                       Default: As defined in $monitor_url
#   $snmp_monitor_process (true|false) : Set if you want to monitor snmp's process. If any.
#                                       Default: As defined in $monitor_process
#   $snmp_monitor_plugin  (true|false) : Set if you want to monitor snmp using specific monitor tool's plugin. If any.
#                                       Default: As defined in $monitor_plugin
#   $snmp_monitor_target               : Define how to reach (Ip, fqdn...) the host to monitor snmp from an external server.
#                                       Default: As defined in $monitor_target
#   $snmp_monitor_url                  : Define the baseurl (http://$fqdn/...) to use for eventual snmp URL checks.
#                                       Default: As defined in $monitor_url
#
...
#
# For the defaults of the above variables check snmp::params
#
# Usage:
...
class snmp::monitor {
    ...
    monitor::port { "snmp_${snmp::params::protocol}_${snmp::params::port}":
        ...
        enable   => "${snmp::params::monitor_port_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::url { "snmp_url":
        ...
        enable   => "${snmp::params::monitor_url_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::process { "snmp_process":
        ...
        enable   => "${snmp::params::monitor_process_enable}",
        tool     => "${monitor_tool}",
    }
    ...
    monitor::plugin { "snmp_plugin":
        plugin   => "snmp",
        enable   => "${snmp::params::monitor_plugin_enable}",
        tool     => "${monitor_tool}",
    }

    # Include project specific monitor class if $my_project is set
    if $my_project { include "snmp::${my_project}::monitor" }
